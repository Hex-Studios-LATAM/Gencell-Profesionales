...
export default function AdminProductosPage() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (!prodRes.ok) throw new Error("Error cargando el catálogo de productos");
    ...
    <main className="p-6 md:p-8 max-w-[1600px] mx-auto w-full">
      ...
      <div className="mb-8 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <h1 className="text-3xl font-extrabold text-slate-900 tracking-tight">Productos</h1>
          <p className="text-slate-500 mt-1 text-sm font-medium">Gestiona tu catálogo médico, activos e inventario visible.</p>
        </div>
        <Link 
          ...
          <table className="w-full text-left border-collapse min-w-[800px]">
            <thead>
              <tr className="bg-slate-50/80 border-b border-slate-200">
                <th className="py-4 px-6 text-xs font-bold text-slate-500 uppercase tracking-wider">Nombre del Producto / Slug</th>
                <th className="py-4 px-6 text-xs font-bold text-slate-500 uppercase tracking-wider w-[25%]">Categoría y Accesos</th>
                ...
                <tr>
                  <td colSpan={4} className="py-20 px-6 text-center">
                    <div className="flex flex-col items-center justify-center gap-3">
                      ...
                      <h3 className="text-slate-900 font-bold text-lg">No hay productos en catálogo</h3>
                      <p className="text-slate-500 text-sm max-w-sm">No tienes productos activos. Ingresa uno nuevo para mostrar a los usuarios profesionales.</p>
                      <Link href="/admin/productos/crear" className="text-indigo-600 font-semibold text-sm hover:text-indigo-700 mt-2 bg-indigo-50 px-4 py-2 rounded-lg transition-colors">
                        Agregar Producto
                      </Link>
                    </div>
                  </td>
                ...
                products.map(prod => (
                  <tr key={prod.id} className="hover:bg-slate-50/60 transition-colors group">
                    <td className="py-4 px-6">
                      <div className="font-bold text-slate-900 text-sm mb-0.5">{prod.name}</div>
                      ...
                      <div className="flex flex-wrap gap-1.5 mb-2">
                        {prod.categories && prod.categories.length > 0 ? (
                           prod.categories.map(c => (
                             <span key={c.id} className="inline-flex items-center px-2 py-1 bg-indigo-50 text-indigo-700 text-xs font-semibold rounded-md border border-indigo-100">
                               {c.name}
                             </span>
                           ))
                        ) : (
                           <span className="inline-flex items-center px-2 py-1 bg-slate-100 text-slate-500 text-xs font-semibold rounded-md border border-slate-200">
                             Sin Categoría
                           </span>
                        )}
                      </div>
                      <div className="flex items-center gap-1.5 flex-wrap">
                         {prod.specialties && prod.specialties.length > 0 
                           ? prod.specialties.map(s => <span key={s.specialty.id} className="bg-white border border-slate-200 shadow-[0_1px_2px_rgba(0,0,0,0.02)] text-slate-600 px-1.5 py-0.5 rounded text-[10px] font-bold tracking-wider">{s.specialty.name}</span>) 
                           : <span className="bg-slate-100 text-slate-500 px-1.5 py-0.5 rounded text-[10px] font-bold tracking-wider">Universal (Todas)</span>
                         }
                      </div>
                    </td>
                    <td className="py-4 px-6">
                    ...
                    <td className="py-4 px-6 text-right">
                      <div className="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button 
                          onClick={() => router.push(`/admin/productos/crear?id=${prod.id}`)} 
